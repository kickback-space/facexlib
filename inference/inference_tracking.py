...
def main(args):
    ...
    for idx, path in enumerate(frame_paths):
        ...
        trackers = tracker.update(
            np.array(face_list), img_size, additional_attr, detect_interval
        )
        ...
        if save_frame:
            for d in trackers:
                d = d.astype(np.int32)
                cv2.rectangle(
                    frame, (d[0], d[1]), (d[2], d[3]), colors[d[4] % 32, :] * 255, 3
                )
                if len(face_list) != 0:
                    cv2.putText(
                        frame,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_folder", help="Path to save visualized frames", type=str, default=None
    )

    parser.add_argument(
        "--detect_interval",
        help=(
            "how many frames to make a detection, trade-off "
